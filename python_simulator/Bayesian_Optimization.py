...
def bayesian_optimization(rounds):
    def evaluate_performance(alpha, beta, gamma): # Objective Function
        global nodes
        ...
        _, std_df, _ = simulate_scheduling(weighted_residual_resource, f"WRR_alpha_{alpha:.2f}_beta_{beta:.2f}_gamma_{gamma:.2f}", round_count, alpha=alpha, beta=beta, gamma=gamma)
        ...
        return -avg_std  # Change the returned value this time to negative because Bayesian Optimization maximizes by default
    ...
    for round_count in rounds:
        ...
        best_std = -optimizer.max['target']

        best_params_per_round.append((round_count, best_std, best_params['alpha'], best_params['beta'], best_params['gamma']))
...
# Define rounds and perform Bayesian Optimization
rounds = list(range(5, 501, 5))
results_df = bayesian_optimization(rounds)
